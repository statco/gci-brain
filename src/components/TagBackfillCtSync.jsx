...
function LogLine({ line, index }) {
  ...
  const isFixed     = line.includes("[fix]");
  const isDry       = line.includes("[dry]");
  ...
  return (
    <div style={{
      ...
      borderLeft: (isFixed || isDry) ? `2px solid ${color}33` : "none",
      paddingLeft: (isFixed || isDry) ? 8 : 0,
...
export default function TagBackfillCtSync() {
  ...
  async function runScan() {
    setScanning(true);
    setScanResult(null);
    setLog([`🔍 Scanning active Tire products${vendor.trim() ? ` — vendor: "${vendor.trim()}"` : ""}…`]);
    try {
      const params = new URLSearchParams({ action: "scan" });
      if (vendor.trim()) params.set("vendor", vendor.trim());
      const res  = await fetch(`/api/tagBackfillCtSync?${params}`);
      const json = await res.json();
      if (json.error) throw new Error(json.error);
      setScanResult(json);
      const breakdown = Object.entries(json.vendorBreakdown || {}).sort(([, a], [, b]) => b - a);
      setLog([
        `✅ Scan complete — ${json.total} total active tires, ${json.missing} missing ct-sync`,
        `──────────────────────────────────────────────────`,
        `📦 Vendor breakdown (${breakdown.length} brand${breakdown.length !== 1 ? 's' : ''} affected):`,
        ...breakdown.slice(0, 30).map(([v, n]) => `  ${v}: ${n}`),
        ...(breakdown.length > 30 ? [`  … and ${breakdown.length - 30} more`] : []),
      ]);
    } catch (err) {
      setLog([`❌ Scan error: ${err.message}`]);
    } finally {
      setScanning(false);
    }
  }
  ...
  async function runFix() {
    ...
    let cumulative    = { scanned: 0, fixed: 0, skipped: 0, errors: 0 };
    ...
    while (true) {
      ...
      try {
        ...
        batchNextOffset = json.nextOffset ?? null;
        setNextOffset(batchNextOffset);
      } catch (err) {
      ...
      if (!canContinue) {
        if (doAutoContinue) {
          if (success && batchSummary && batchSummary.scanned === 0) {
            setLog(l => [...l, "✅ All products tagged."]);
          }
  ...
  const vendorParams = vendor.trim() ? `&vendor=${encodeURIComponent(vendor.trim())}` : "";
      ...
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 32, animation: "fadeIn 0.4s ease" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
            ...
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: "-0.02em" }}>GCI Tires — Tag Backfill: ct-sync</div>
              <div style={{ fontSize: 12, color: "#555", fontFamily: "monospace" }}>gci-brain.vercel.app/api/tagBackfillCtSync</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 24, animation: "fadeIn 0.4s ease 0.1s both" }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 12, color: "#666", display: "block", marginBottom: 8 }}>Vendor Filter <span style={{ color: "#444" }}>(optional)</span></label>
              <div style={{ display: "flex", gap: 6 }}>
                ...
                <button
                  onClick={runScan}
                  disabled={scanning}
                  style={{ background: scanning ? "#1a1a1a" : "rgba(52,211,153,0.12)", border: "1px solid rgba(52,211,153,0.3)", borderRadius: 7, color: scanning ? "#444" : "#34d399", fontSize: 11, fontWeight: 600, padding: "0 14px", cursor: scanning ? "not-allowed" : "pointer", whiteSpace: "nowrap" }}
                >
                  {scanning ? <span style={{ display: "inline-block", animation: "spin 0.8s linear infinite" }}>⟳</span> : "Scan"}
                </button>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 12, color: "#666", display: "block", marginBottom: 8 }}>
                Chunk Size — <span style={{ color: "#60a5fa" }}>{chunkSize} products</span>
              </label>
              <input type="range" min={10} max={250} step={10} value={chunkSize}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#444", marginTop: 4 }}>
                <span>10</span><span>130</span><span>250</span>
              </div>
            </div>
            ...
            {/* Dry/Live toggle */}
            <div
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", background: dryRun ? "rgba(96,165,250,0.06)" : "rgba(239,68,68,0.06)", border: `1px solid ${dryRun ? "rgba(96,165,250,0.2)" : "rgba(239,68,68,0.2)"}`, borderRadius: 9, marginBottom: 12, cursor: "pointer" }}
              onClick={() => setDryRun(d => !d)}
            >
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: dryRun ? "#60a5fa" : "#f87171" }}>{dryRun ? "🔍 Dry Run Mode" : "⚡ Live Mode"}</div>
                <div style={{ fontSize: 11, color: "#555", marginTop: 2 }}>{dryRun ? "No writes — preview only" : "Will write to Shopify"}</div>
              </div>
              <div style={{ width: 36, height: 20, borderRadius: 10, background: dryRun ? "#60a5fa" : "#ef4444", position: "relative", transition: "background 0.2s" }}>
            ...
            <div
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", background: autoContinue ? "rgba(96,165,250,0.06)" : "rgba(255,255,255,0.02)", border: `1px solid ${autoContinue ? "rgba(96,165,250,0.2)" : "rgba(255,255,255,0.06)"}`, borderRadius: 9, marginBottom: 20, cursor: "pointer" }}
              onClick={() => setAutoContinue(a => !a)}
            >
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: autoContinue ? "#60a5fa" : "#555" }}>🔁 Auto-Continue</div>
                <div style={{ fontSize: 11, color: "#555", marginTop: 2 }}>Runs all batches automatically</div>
              </div>
              <div style={{ width: 36, height: 20, borderRadius: 10, background: autoContinue ? "#60a5fa" : "#333", position: "relative", transition: "background 0.2s" }}>
            ...
            {isAutoRunning ? (
              ...
              <button onClick={runFix} disabled={running} style={{ width: "100%", background: running ? "#1a1a1a" : "linear-gradient(135deg, #34d399, #059669)", border: "none", borderRadius: 10, padding: "13px 0", color: running ? "#444" : "#fff", fontSize: 14, fontWeight: 700, cursor: running ? "not-allowed" : "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s ease", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                {running ? <><span style={{ display: "inline-block", animation: "spin 0.8s linear infinite", fontSize: 16 }}>⟳</span>Processing…</> : `▶ Run Fix${dryRun ? " (Dry Run)" : ""}`}
              </button>
            )}

            {nextOffset != null && !running && !isAutoRunning && (
              <button onClick={() => { setOffset(nextOffset); setNextOffset(null); }} style={{ width: "100%", marginTop: 8, background: "rgba(52,211,153,0.1)", border: "1px solid rgba(52,211,153,0.3)", borderRadius: 10, padding: "11px 0", color: "#34d399", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {/* Scan results */}
            {scanResult && (
              <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 24, animation: "fadeIn 0.4s ease both" }}>
                <div style={{ fontSize: 11, color: "#555", textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: 12 }}>Scan Results</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
                  <StatCard label="Total Tires" value={scanResult.total} color="#9ca3af" />
                  <StatCard label="Missing ct-sync" value={scanResult.missing} color="#f59e0b" />
                </div>
                {Object.keys(scanResult.vendorBreakdown || {}).length > 0 && (
                  <>
                    <div style={{ fontSize: 10, color: "#444", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 8, fontFamily: "monospace" }}>Missing by Vendor</div>
                    <div style={{ maxHeight: 150, overflowY: "auto" }}>
                      {Object.entries(scanResult.vendorBreakdown)
                        .sort(([, a], [, b]) => b - a)
                        .map(([v, n]) => (
                          <div key={v} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12, padding: "4px 0", borderBottom: "1px solid #1a1a1a", fontFamily: "monospace" }}>
                            <span style={{ color: "#9ca3af", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "70%" }}>{v}</span>
                            <span style={{ color: "#f59e0b", fontWeight: 600, flexShrink: 0 }}>{n}</span>
                          </div>
                        ))}
                    </div>
                  </>
            ...
            <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 24, animation: "fadeIn 0.4s ease 0.15s both" }}>
              <div style={{ fontSize: 11, color: "#555", textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: 16 }}>Fix Summary</div>
              {summary ? (
                <>
                  {cumulativeSummary && (
                    <>
                ...
                <div style={{ height: 100, display: "flex", alignItems: "center", justifyContent: "center", color: "#333", fontSize: 13, fontStyle: "italic", textAlign: "center", lineHeight: 1.6 }}>
                  {running ? <span style={{ animation: "pulse 1s ease infinite" }}>Running…</span> : "Scan first to preview,\nthen click Run Fix"}
                </div>
              )}
            </div>
            ...
            <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 16, animation: "fadeIn 0.4s ease 0.2s both" }}>
              ...
              <div style={{ fontFamily: "monospace", fontSize: 11, color: "#60a5fa", background: "#0a0a0a", border: "1px solid #1a1a1a", borderRadius: 7, padding: "8px 12px", wordBreak: "break-all", lineHeight: 1.6 }}>
                /api/tagBackfillCtSync?action=fix&chunkSize={chunkSize}&offset={offset}&dry={dryRun ? "true" : "false"}{vendorParams}
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginTop: 20, background: "#080808", border: "1px solid #1a1a1a", borderRadius: 14, overflow: "hidden", animation: "fadeIn 0.4s ease 0.25s both" }}>
          <div style={{ padding: "12px 20px", borderBottom: "1px solid #1a1a1a", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: (running || isAutoRunning || scanning) ? "#34d399" : log.length ? "#fbbf24" : "#333", animation: (running || isAutoRunning || scanning) ? "pulse 1s ease infinite" : "none" }} />
          ...
          <div ref={logRef} style={{ height: 320, overflowY: "auto", padding: "16px 20px" }}>
            {log.length === 0
              ? <div style={{ color: "#2a2a2a", fontFamily: "monospace", fontSize: 12 }}>$ waiting for scan or run…</div>
