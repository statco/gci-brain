...
function LogLine({ line, index }) {
  ...
  const isDone     = line.includes("✅ All titles fixed.");
...
export default function FixTitles() {
  ...
  async function runFix() {
    ...
    while (true) {
      ...
      try {
        ...
        const res  = await fetch(`/api/fixTitles?${params}`);
      ...
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 32, animation: "fadeIn 0.4s ease" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
            <div style={{ width: 36, height: 36, borderRadius: 8, background: "linear-gradient(135deg, #60a5fa, #2563eb)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>🔤</div>
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: "-0.02em" }}>GCI Tires — Fix Titles</div>
              <div style={{ fontSize: 12, color: "#555", fontFamily: "monospace" }}>gci-brain.vercel.app/api/fixTitles</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 24, animation: "fadeIn 0.4s ease 0.1s both" }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="range" min={5} max={100} step={5} value={chunkSize}
            ...
            {isAutoRunning ? (
              ...
              <button onClick={runFix} disabled={running} style={{ width: "100%", background: running ? "#1a1a1a" : "linear-gradient(135deg, #60a5fa, #2563eb)", border: "none", borderRadius: 10, padding: "13px 0", color: running ? "#444" : "#fff", fontSize: 14, fontWeight: 700, cursor: running ? "not-allowed" : "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s ease", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 16, animation: "fadeIn 0.4s ease 0.2s both" }}>
              ...
              <div style={{ fontFamily: "monospace", fontSize: 11, color: "#60a5fa", background: "#0a0a0a", border: "1px solid #1a1a1a", borderRadius: 7, padding: "8px 12px", wordBreak: "break-all", lineHeight: 1.6 }}>
                /api/fixTitles?chunkSize={chunkSize}&offset={offset}{dryRun ? "&dryRun=true" : ""}
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginTop: 20, background: "#080808", border: "1px solid #1a1a1a", borderRadius: 14, overflow: "hidden", animation: "fadeIn 0.4s ease 0.25s both" }}>
          <div style={{ padding: "12px 20px", borderBottom: "1px solid #1a1a1a", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: running || isAutoRunning ? "#60a5fa" : log.length ? "#fbbf24" : "#333", animation: running || isAutoRunning ? "pulse 1s ease infinite" : "none" }} />
