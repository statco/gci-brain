...
function TagProductsCard() {
  ...
  const run = async (preview = false) => {
    ...
    try {
      while (true) {
        const url = `/api/tagProducts?cursor=${cursor}${preview ? '&preview=true' : ''}`;
        const resp = await fetch(url);
        if (!resp.ok) throw new Error(`Server error ${resp.status}`);
        const data = await resp.json();
    ...
    <div style={tsfc.card} className="dash-card">
      ...
      <div style={tsfc.top}>
        <div style={tsfc.titleRow}>
          ...
          <h3 style={styles.cardTitle}>Tag Products</h3>
        </div>
        <span style={isRunning ? tsfc.badgeRunning : status === 'done' ? tsfc.badgeDone : tsfc.badgeIdle}>
          {isRunning ? '⟳ RUNNING' : status === 'done' ? '✓ DONE' : status === 'error' ? '✗ ERROR' : '● READY'}
        </span>
      ...
      <p style={styles.cardDesc}>
        Auto-assign GMC custom label tags (season:Winter, vehicle_type:Passenger, etc.) from product title keywords. Runs in 50-product chunks.
      </p>
      ...
      {(isRunning || status === 'done' || status === 'error') && (
        <div style={tsfc.progressWrap}>
          <div style={tsfc.progressLabel}>
            {isRunning
              ? `Tagged ${progress.fixed} / ${progress.total} — ${progress.skipped} skipped…`
              : `Tagged ${progress.fixed} / ${progress.total} — ${progress.skipped} skipped${errors.length ? ` — ${errors.length} error(s)` : ''}`
            }
          </div>
          {progress.total > 0 && (
            <div style={tsfc.barTrack}>
      ...
      {previewItems.length > 0 && (
        <div style={tsfc.previewWrap}>
          <div style={tsfc.previewHeading}>PREVIEW — first {previewItems.length} products to tag</div>
          <div style={tsfc.previewList}>
            {previewItems.map((item, i) => (
              <div key={i} style={tsfc.previewRow}>
                <span style={tsfc.previewBefore}>{item.title}</span>
                <span style={tsfc.previewArrow}>+</span>
                <span style={tsfc.previewAfter}>{item.addedTags.join(', ')}</span>
              </div>
            ))}
          </div>
        </div>
