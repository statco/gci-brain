...
function StatCard({ label, value, accent }) {
  return (
    <div style={{
      background: accent ? "rgba(248,113,113,0.08)" : "rgba(255,255,255,0.03)",
      border: `1px solid ${accent ? "rgba(248,113,113,0.3)" : "rgba(255,255,255,0.07)"}`,
      borderRadius: 10,
      padding: "14px 18px",
      display: "flex",
...
function LogLine({ line, index }) {
  ...
  const isTranslated = line.includes("✅") && !isDone;
  ...
  return (
    <div style={{
      ...
      borderLeft: isTranslated ? `2px solid ${color}33` : "none",
      paddingLeft: isTranslated ? 8 : 0,
...
export default function FixProductDescriptions() {
  ...
  async function runFix() {
    ...
    let cumulative    = { scanned: 0, translated: 0, skipped: 0, errors: 0 };
    ...
    while (true) {
      ...
      try {
        ...
        const res  = await fetch(`/api/fixProductDescriptions?${params}`);
      ...
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 32, animation: "fadeIn 0.4s ease" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
            <div style={{ width: 36, height: 36, borderRadius: 8, background: "linear-gradient(135deg, #a78bfa, #7c3aed)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>EN</div>
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: "-0.02em" }}>GCI Tires — Product Description Translator</div>
              <div style={{ fontSize: 12, color: "#555", fontFamily: "monospace" }}>gci-brain.vercel.app/api/fixProductDescriptions</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 24, animation: "fadeIn 0.4s ease 0.1s both" }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 12, color: "#666", display: "block", marginBottom: 8 }}>
                Chunk Size — <span style={{ color: "#a78bfa" }}>{chunkSize} products</span>
              </label>
              <input
                type="range"
                min={10}
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                ...
                <button
                  onClick={() => setOffset(0)}
                  style={{ background: "#1a1a1a", border: "1px solid #222", borderRadius: 7, color: "#555", fontSize: 11, padding: "0 12px", cursor: "pointer" }}
                >
                  Reset
                </button>
              </div>
            ...
            {/* Dry Run toggle */}
            <div
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", background: dryRun ? "rgba(251,191,36,0.06)" : "rgba(239,68,68,0.06)", border: `1px solid ${dryRun ? "rgba(251,191,36,0.2)" : "rgba(239,68,68,0.2)"}`, borderRadius: 9, marginBottom: 12, cursor: "pointer" }}
            ...
            {isAutoRunning ? (
              ...
              <button
                onClick={runFix}
                disabled={running}
                style={{ width: "100%", background: running ? "#1a1a1a" : "linear-gradient(135deg, #a78bfa, #7c3aed)", border: "none", borderRadius: 10, padding: "13px 0", color: running ? "#444" : "#fff", fontSize: 14, fontWeight: 700, cursor: running ? "not-allowed" : "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s ease", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
              >
                {running
            ...
            {nextOffset != null && !running && !isAutoRunning && (
              <button
                onClick={() => { setOffset(nextOffset); setNextOffset(null); }}
                style={{ width: "100%", marginTop: 8, background: "rgba(167,139,250,0.1)", border: "1px solid rgba(167,139,250,0.3)", borderRadius: 10, padding: "11px 0", color: "#a78bfa", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}
              >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {summary ? (
                <>
                    ...
                    <>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
                        {Object.entries(STAT_LABELS).map(([key, label]) => (
                          <StatCard key={key} label={label} value={cumulativeSummary[key]} accent={key === "errors" && cumulativeSummary[key] > 0} />
                        ))}
                      </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    {Object.entries(STAT_LABELS).map(([key, label]) => (
                      <StatCard key={key} label={label} value={summary[key]} accent={key === "errors" && summary[key] > 0} />
                    ))}
                  </div>
                </>
              ) : (
                <div style={{ height: 120, display: "flex", alignItems: "center", justifyContent: "center", color: "#333", fontSize: 13, fontStyle: "italic" }}>
                  {running
                    ...
                    : "No run yet — configure and click Run"}
                </div>
              )}
            </div>
            ...
            <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 16, animation: "fadeIn 0.4s ease 0.2s both" }}>
              ...
              <div style={{ fontFamily: "monospace", fontSize: 11, color: "#a78bfa", background: "#0a0a0a", border: "1px solid #1a1a1a", borderRadius: 7, padding: "8px 12px", wordBreak: "break-all", lineHeight: 1.6 }}>
                /api/fixProductDescriptions?chunkSize={chunkSize}&offset={offset}{dryRun ? "&dryRun=true" : ""}
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginTop: 20, background: "#080808", border: "1px solid #1a1a1a", borderRadius: 14, overflow: "hidden", animation: "fadeIn 0.4s ease 0.25s both" }}>
          <div style={{ padding: "12px 20px", borderBottom: "1px solid #1a1a1a", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: running || isAutoRunning ? "#34d399" : log.length ? "#a78bfa" : "#333", animation: running || isAutoRunning ? "pulse 1s ease infinite" : "none" }} />
