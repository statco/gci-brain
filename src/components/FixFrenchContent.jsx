...
function LogLine({ line, index }) {
  ...
  return (
    <div style={{
      ...
      borderLeft: isFixed || isUpdate ? `2px solid ${color}33` : "none",
      paddingLeft: isFixed || isUpdate ? 8 : 0,
...
export default function FixFrenchContent() {
  ...
  async function runFix() {
    setRunning(true);
    setLog([`▶ Starting — scope=${scope}${dryRun ? " [DRY RUN]" : " [LIVE]"}…`]);
    setSummary(null);

    try {
      const params = new URLSearchParams({ scope, ...(dryRun ? { dryRun: "true" } : {}) });
      const res  = await fetch(`/api/fixFrenchContent?${params}`);
      ...
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 32, animation: "fadeIn 0.4s ease" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
            <div style={{ width: 36, height: 36, borderRadius: 8, background: "linear-gradient(135deg, #60a5fa, #3b82f6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>FR</div>
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: "-0.02em" }}>GCI Tires — French Content Fixer</div>
              <div style={{ fontSize: 12, color: "#555", fontFamily: "monospace" }}>gci-brain.vercel.app/api/fixFrenchContent</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 24, animation: "fadeIn 0.4s ease 0.1s both" }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 12, color: "#666", display: "block", marginBottom: 8 }}>Scope</label>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
                {SCOPES.map(s => (
                  <button key={s.id} onClick={() => setScope(s.id)} style={{
                    background: scope === s.id ? "rgba(96,165,250,0.12)" : "rgba(255,255,255,0.03)",
                    border: `1px solid ${scope === s.id ? "rgba(96,165,250,0.4)" : "rgba(255,255,255,0.06)"}`,
                    borderRadius: 8, padding: "8px 10px",
                    color: scope === s.id ? "#60a5fa" : "#888",
                    fontSize: 12, fontWeight: scope === s.id ? 600 : 400,
                    cursor: "pointer", textAlign: "center", transition: "all 0.15s ease", fontFamily: "'DM Sans', sans-serif",
                  }}>{s.label}</button>
                ))}
              </div>
            </div>
            ...
            <button
              onClick={runFix}
              disabled={running}
              style={{ width: "100%", background: running ? "#1a1a1a" : "linear-gradient(135deg, #60a5fa, #3b82f6)", border: "none", borderRadius: 10, padding: "13px 0", color: running ? "#444" : "#000", fontSize: 14, fontWeight: 700, cursor: running ? "not-allowed" : "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s ease", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
            >
              {running
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 24, animation: "fadeIn 0.4s ease 0.15s both" }}>
              ...
              {summary ? (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  ...
                  <StatCard label="Errors"  value={summary.errors} accent={summary.errors > 0} />
                </div>
              ) : (
            ...
            <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 16, animation: "fadeIn 0.4s ease 0.2s both" }}>
              ...
              <div style={{ fontFamily: "monospace", fontSize: 11, color: "#60a5fa", background: "#0a0a0a", border: "1px solid #1a1a1a", borderRadius: 7, padding: "8px 12px", wordBreak: "break-all", lineHeight: 1.6 }}>
                /api/fixFrenchContent?scope={scope}{dryRun ? "&dryRun=true" : ""}
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginTop: 20, background: "#080808", border: "1px solid #1a1a1a", borderRadius: 14, overflow: "hidden", animation: "fadeIn 0.4s ease 0.25s both" }}>
          <div style={{ padding: "12px 20px", borderBottom: "1px solid #1a1a1a", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: running ? "#34d399" : log.length ? "#60a5fa" : "#333", animation: running ? "pulse 1s ease infinite" : "none" }} />
