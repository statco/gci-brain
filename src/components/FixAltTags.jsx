...
function LogLine({ line, index }) {
  ...
  const isTranslate = line.includes("[translate]");
  const isGenerate  = line.includes("[generate]");
  const isNext      = line.includes("Next chunk");
  ...
  return (
    <div style={{
      ...
      borderLeft: isTranslate || isGenerate ? `2px solid ${color}33` : "none",
      paddingLeft: isTranslate || isGenerate ? 8 : 0,
...
export default function FixAltTags() {
  ...
  async function runFix() {
    ...
    while (true) {
      ...
      try {
        ...
        const res  = await fetch(`/api/fixAltTags?${params}`);
  ...
  const STAT_LABELS = { scanned: "Scanned", fixed: "Fixed", skipped: "Skipped", errors: "Errors" };
      ...
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        <div style={{ marginBottom: 32, animation: "fadeIn 0.4s ease" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
            <div style={{ width: 36, height: 36, borderRadius: 8, background: "linear-gradient(135deg, #fbbf24, #f59e0b)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>🖼️</div>
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: "-0.02em" }}>GCI Tires — Fix Alt Tags</div>
              <div style={{ fontSize: 12, color: "#555", fontFamily: "monospace" }}>gci-brain.vercel.app/api/fixAltTags</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 24, animation: "fadeIn 0.4s ease 0.1s both" }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="range" min={10} max={50} step={5} value={chunkSize}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 16, animation: "fadeIn 0.4s ease 0.2s both" }}>
              ...
              <div style={{ fontFamily: "monospace", fontSize: 11, color: "#60a5fa", background: "#0a0a0a", border: "1px solid #1a1a1a", borderRadius: 7, padding: "8px 12px", wordBreak: "break-all", lineHeight: 1.6 }}>
                /api/fixAltTags?chunkSize={chunkSize}&offset={offset}{dryRun ? "&dryRun=true" : ""}
              </div>
            </div>
          </div>
        </div>
