...
const CheckoutModal: React.FC<CheckoutModalProps> = ({ 
  ...
  const handleProceedToCheckout = async () => {
    ...
    try {
      // Validate variant ID
      if (!tire.variantId || tire.variantId.startsWith('mock') || tire.variantId === '0') {
        throw new Error('Invalid product variant. Please try another tire.');
      }
      ...
      const checkoutUrl = await createShopifyCheckout(
        ...
        quantity,
        withInstallation,
        tire.installationFeePerUnit
      );

      // Redirect to Shopify checkout
      window.location.href = checkoutUrl;
      
      // Call onConfirm after redirect is initiated
      onConfirm();
      ...
      <div className="fixed inset-0 bg-slate-900 bg-opacity-95 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
        <div className="bg-white rounded-lg p-10 max-w-sm w-full text-center shadow-2xl">
          ...
          <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">
            {t.redirectTitle || 'Creating Checkout'}
          </h3>
          <p className="text-slate-500 mt-2 font-medium">
            {t.redirectMsg || 'Please wait...'}
          </p>
          <div className="mt-4 text-xs text-slate-400">Securing your transaction...</div>
        </div>
      </div>
    );
  }
      ...
      <div className="fixed inset-0 bg-slate-900 bg-opacity-75 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
        <div className="bg-white rounded-lg shadow-2xl max-w-md w-full overflow-hidden animate-fade-in-up border-t-4 border-red-600">
          <div className="p-6 text-center">
            ...
            <h3 className="text-xl font-black text-slate-900 mb-2">Checkout Error</h3>
      ...
      <div className="bg-white rounded-lg shadow-2xl max-w-lg w-full overflow-hidden animate-fade-in-up border-t-4 border-red-600">
        ...
        <div className="bg-white border-b border-slate-200 p-6 flex justify-between items-center">
          <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">
            {t.reviewSelection || 'Review Selection'}
          </h2>
        ...
        <div className="p-6">
          ...
          <div className="border-t border-slate-100 py-4 space-y-2 mb-6">
            <div className="flex justify-between text-sm text-slate-600 font-medium">
              <span>Subtotal ({quantity} tires)</span>
              <span>${(tire.pricePerUnit * quantity).toFixed(2)}</span>
            </div>
            {withInstallation && (
              <div className="flex justify-between text-sm text-slate-600 font-medium">
                <span>Installation Service ({quantity}x)</span>
          ...
          <div className="space-y-4">
            ...
            >
              ...
              {t.proceedCheckout || 'Proceed to Secure Checkout'}
            </button>
